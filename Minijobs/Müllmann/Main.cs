...
namespace Altv_Roleplay.Minijobs.Müllmann
{
    public class Main
    {
        ...
        public static void Initialize()
        {
            ...
            {
                model = "",
                posX = startJobShape.Position.X,
                posY = startJobShape.Position.Y,
                posZ = startJobShape.Position.Z,
                rotation = -0.9688125252723694f
            };
            ...
            {
                type = 33,
                posX = Constants.Positions.Minijob_Müllmann_VehOutPos.X,
                posY = Constants.Positions.Minijob_Müllmann_VehOutPos.Y,
                posZ = Constants.Positions.Minijob_Müllmann_VehOutPos.Z + 1,
                alpha = 150,
                bobUpAndDown = true,
                scaleX = 1,
                scaleY = 1,
        ...
        private static async void PlayerExitVehicle(IVehicle vehicle, IPlayer player, byte seat)
        {
            try
            {
                ...
                {
                    HUDHandler.SendNotification(player, 3, 5000, $"Dein Gehalt i.H.v. {rnd}$ konnte nicht überwiesen werden da du kein Hauptkonto hast.");
                    return;
                }
