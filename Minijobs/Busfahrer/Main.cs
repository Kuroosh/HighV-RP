    ...
    public class Main : IScript
    {
        ...
        public static async void PlayerExitVehicle(ClassicVehicle vehicle, ClassicPlayer player, byte seat)
        {
            try
            {
                ...
                {
                    HUDHandler.SendNotification(player, 3, 5000, $"Dein Gehalt i.H.v. {givenMoney}$ konnte nicht überwiesen werden da du kein Hauptkonto hast. Du hast aber {givenEXP}EXP erhalten (du hast nun: {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")}EXP).");
                    return;
                }
