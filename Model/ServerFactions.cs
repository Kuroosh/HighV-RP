    ...
    class ServerFactions
    {
        ...
        public static int GetFactionDutyMemberCount(int factionId)
        {
            ...
            try
            {
                ...
                memberCount = ServerFactionMembers_.Where(x => x.factionId == factionId && x.isDuty == true && Alt.Server.GetPlayers().ToList().FirstOrDefault(p => p.GetCharacterMetaId() == (ulong)x.charId) != null).Count();
            }
            catch (Exception e)
            {
                Core.Debug.CatchExceptions(e);
