    ...
    class HouseHandler : IScript
    {
        ...
        public static void RemoveRenter(ClassicPlayer player, int houseId, int renterId)
        {
            try
            {
                ...
                foreach (var renterPlayer in Alt.Server.GetPlayers().ToList().Where(x => x != null && x.Exists && x.GetCharacterMetaId() == (ulong)renterId)) { HUDHandler.SendNotification(renterPlayer, 3, 2000, $"Dein Mietvertrag in dem Haus '{ServerHouses.GetHouseStreet(houseId)}' wurde gekündigt."); break; }
            }
            catch (Exception e)
            {
                Core.Debug.CatchExceptions(e);
