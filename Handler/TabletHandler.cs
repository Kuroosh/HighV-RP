    ...
    class TabletHandler : IScript
    {
        ...
        public static void VehicleStoreBuyVehicle(ClassicPlayer player, string hash, int shopId, string color)
        {
            try
            {
                ...
                switch (color)
                {
                    case "schwarz": fColor = 0; break;
                    case "weiß": fColor = 112; break;
                    case "blau": fColor = 64; break;
                    case "rot": fColor = 27; break;
                    case "grün": fColor = 57; break;
                    case "gelb": fColor = 42; break;
                }
        ...
        public static void sendDispatchToFaction(ClassicPlayer player, int factionId, string msg)
        {
            try
            {
                ...
                foreach (var p in Alt.Server.GetPlayers().Where(x => x != null && x.Exists && x.GetCharacterMetaId() > 0).ToList())
                {
                    ...
                    HUDHandler.SendNotification(p, 1, 3500, "Ein neuer Notruf ist eingegangen.");
                }
            }
            catch (Exception e)
            {
